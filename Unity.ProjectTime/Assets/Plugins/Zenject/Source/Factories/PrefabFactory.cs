...
namespace Zenject
{
        ...
        //where T : Component
    {
        ...
        public virtual T Create(UnityEngine.Object unitType, P1 spawnTileId)
        {
            Assert.That(unitType != null,
               "Null prefab given to factory create method when instantiating object with type '{0}'.", typeof(T));

            return (T)_container.InstantiatePrefabForComponentExplicit(
                typeof(T), unitType, InjectUtil.CreateArgListExplicit(spawnTileId));
        }
    }
