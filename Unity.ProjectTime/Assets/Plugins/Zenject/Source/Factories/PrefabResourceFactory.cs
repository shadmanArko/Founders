...
namespace Zenject
{
        ...
        //where T : Component
    {
        ...
        public virtual T Create(string unitType, P1 spawnTileId)
        {
            Assert.That(!string.IsNullOrEmpty(unitType),
              "Null or empty prefab resource name given to factory create method when instantiating object with type '{0}'.", typeof(T));

            var prefab = (GameObject)Resources.Load(unitType);
            return (T)_container.InstantiatePrefabForComponentExplicit(
                typeof(T), prefab, InjectUtil.CreateArgListExplicit(spawnTileId));
        }
    }
